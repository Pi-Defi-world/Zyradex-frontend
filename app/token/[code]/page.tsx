...
import { ArrowRightLeft, ArrowUpRight, ArrowDown, Loader2, Copy, ExternalLink, Plus, Users, Droplets, Coins } from "lucide-react"
...
export default function TokenDetailPage({ params }: { params: Promise<{ code: string }> }) {
  ...
  const tokenCode = decodeURIComponent(code)
  const issuer = searchParams.get("issuer") || undefined
  const isNative = tokenCode === "native"

  // Fetch metadata for non-platform tokens
  useEffect(() => {
    if (isNative || !issuer) return
    const found = lookup(tokenCode, issuer)
    if (found?.name || found?.image) {
      setFetchedMeta(found)
      return
    }
    let cancelled = false
    fetchMetadata(tokenCode, issuer).then((m) => {
      if (!cancelled && m) setFetchedMeta(m)
    })
    return () => { cancelled = true }
  }, [tokenCode, issuer, isNative, lookup, fetchMetadata])
  ...
  const { balances, isLoading: balancesLoading, refresh: refreshBalances } = useAccountBalances(publicKey)
  const { operations, isLoading: operationsLoading } = useAccountOperations(publicKey, { limit: 50 })

  const tokenBalance = useMemo(() => {
    if (isNative) return balances.find((b) => b.assetType === "native")
    return balances.find(
      (b) => b.assetCode === tokenCode && (issuer ? b.assetIssuer === issuer : !b.assetIssuer)
    )
  }, [balances, tokenCode, issuer, isNative])

  const amount = tokenBalance ? Number(tokenBalance.amount) : 0

  const { hasTrustline, isLoading: trustlineLoading, refresh: refreshTrustline } = useCheckTrustline(
    publicKey,
    tokenCode,
    issuer,
    !isNative
  ...
  const tokenOperations = useMemo(() => {
    ...
    const tokenAsset = isNative ? "native" : issuer ? `${tokenCode}:${issuer}` : tokenCode
    return operations.filter((op) => {
      const opAsset = op.asset || ""
      const opSelling = op.selling || ""
      const opBuying = op.buying || ""
      if (isNative) {
        return opAsset.includes("native") || opSelling.includes("native") || opBuying.includes("native") || op.type === "payment" || op.type === "createAccount"
      }
      return opAsset.includes(tokenCode) || opSelling.includes(tokenCode) || opBuying.includes(tokenCode) || op.asset?.includes(tokenAsset) || op.from?.includes(tokenAsset) || op.to?.includes(tokenAsset)
    })
  ...
  const meta = !isNative ? lookup(tokenCode, issuer) : undefined
  const resolvedMeta = fetchedMeta || meta
  const tokenMetaName = resolvedMeta?.name || tokenCode
  const tokenMetaDesc = resolvedMeta?.description
  const tokenMetaHolders = resolvedMeta?.holders
    ...
    <div className="min-h-screen premium-gradient pt-24 pb-24 lg:pb-6">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <TokenIcon image={resolvedMeta?.image ?? meta?.image} code={isNative ? "PI" : tokenCode} issuer={issuer} size="lg" />
                  <div>
                    <h1 className="text-3xl font-bold text-foreground mb-1">{displayName}</h1>
                    {tokenMetaName !== tokenCode && (
                      <p className="text-sm text-muted-foreground">{tokenMetaName}</p>
                    )}
                    {!isNative && issuer && (
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <button onClick={() => handleCopy(issuer)} className="text-muted-foreground hover:text-foreground transition-colors" title="Copy issuer">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {tokenMetaSupply !== undefined && (
                    <div className="rounded-xl bg-muted/30 p-3 text-center">
                      ...
                      <p className="text-lg font-bold">{tokenMetaSupply.toLocaleString(undefined, { maximumFractionDigits: 0 })}</p>
                      <p className="text-[10px] text-muted-foreground">Circulating</p>
                    </div>
                  )}
                  {tokenMetaHolders !== undefined && tokenMetaHolders > 0 && (
                    <div className="rounded-xl bg-muted/30 p-3 text-center">
                      <Users className="h-4 w-4 mx-auto mb-1 text-muted-foreground" />
                      <p className="text-lg font-bold">{tokenMetaHolders.toLocaleString()}</p>
                      <p className="text-[10px] text-muted-foreground">Holders</p>
                    </div>
                  )}
                  {tokenMetaLiquidity !== undefined && (
                    <div className="rounded-xl bg-muted/30 p-3 text-center">
                      <Droplets className="h-4 w-4 mx-auto mb-1 text-muted-foreground" />
                      <p className="text-lg font-bold">{tokenMetaLiquidity}</p>
              ...
              {tokenMetaDesc && (
                <p className="text-sm text-muted-foreground leading-relaxed">{tokenMetaDesc}</p>
              )}
              ...
              <div className="pt-2">
                {!isAuthenticated ? (
                  <Button onClick={() => router.push("/profile")} className="w-full bg-gradient-to-r from-green-600 to-mint-500 text-white">
                    Connect Wallet to interact
                  </Button>
                ) : !isNative && trustlineLoading ? (
                  ...
                  <Button
                    ...
                    className="w-full h-14 text-base gap-2 rounded-xl bg-gradient-to-r from-emerald-500 to-green-500 hover:from-emerald-600 hover:to-green-600 text-white shadow-lg"
                  >
                    <Plus className="h-5 w-5" />
                    Add {displayName} Trustline
                  </Button>
                ) : (
              ...
              {operationsLoading ? (
                ...
                <div className="space-y-2">
                  {tokenOperations.map((op) => (
                    <div key={op.id} className="flex items-center justify-between p-4 rounded-xl bg-muted/30 border border-border/50 hover:bg-muted/60 transition-colors">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <p className="font-semibold text-foreground capitalize">
                            {op.type.replace(/([A-Z])/g, " $1").trim()}
                          </p>
                          {op.action && <span className="text-xs text-muted-foreground">({op.action})</span>}
                        </div>
                        <p className="text-xs text-muted-foreground">
                          {op.createdAt ? new Date(op.createdAt).toLocaleString(undefined, { month: "short", day: "numeric", year: "numeric", hour: "2-digit", minute: "2-digit" }) : "Unknown date"}
                        </p>
                        {op.amount && (
                      ...
                      {op.transactionHash && (
                        <Button variant="ghost" size="sm" onClick={() => viewTransactionOnExplorer(op.transactionHash!)} className="ml-4 shrink-0" title="View on explorer">
      ...
      {!isNative && (
        <TrustlineDialog
          ...
          onOpenChange={setTrustlineDialogOpen}
          token={{
            assetCode: tokenCode,
            issuer: issuer || "",
            name: tokenMetaName,
